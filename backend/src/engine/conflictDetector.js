...
export const runConflictDetection = async (projectId, jobId, onProgress = null) => {
    try {
        ...
        let parsed;
        try {
            parsed = await parseRequirements(projectId);
        } catch (err) {
            // If no requirements, we can't do analysis but it's not a "fatal" engine crash
            progress(projectId, jobId, onProgress, 100, `Complete: No requirements found yet.`);
            return { projectId, jobId, totalConflicts: 0 };
        }

        if (parsed.length < 2) {
            progress(projectId, jobId, onProgress, 100, `Complete: ${parsed.length} requirement found. Need 2+ for conflict analysis.`);
            return { projectId, jobId, totalConflicts: 0, requirementsAnalyzed: parsed.length };
        }
        ...
        console.error(`[RCFA ERROR] Analysis failed for project ${projectId}:`, error);
        progress(projectId, jobId, onProgress, 100, `Analysis failed: ${error.message}`);
        throw error;
    }
};
